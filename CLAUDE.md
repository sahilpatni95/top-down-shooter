...
2. **SETTINGS** — `settings` object (including `moveMode: 'keyboard'|'mouse'`, `playerIcon: 'circle'|'triangle'|'diamond'|'square'|'star'`) + UI event wiring for the bottom panel; 7-column CSS grid (columns: Name, Background, Difficulty, Volume, Player Color, Movement+Fullscreen, Icon); CSS media queries added for responsive layout: ≤600px → 2-col, ≤900px → 4-col, `touch-action:none` on canvas
...
- **Touch joystick** — `handleTouches()` called from touchstart/move/end listeners; left-half touches set `joystick.{active,startX,startY,dx,dy}`; `updatePlayer()` reads joystick in both moveMode branches; right-half touches feed into `mouse.{x,y,held}` directly
